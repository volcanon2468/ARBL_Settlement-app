...
def build_shutdown_blocks(shutdown_windows: List[Dict]) -> Set[Tuple[date, int]]:
    ...
    for window in shutdown_windows:
        dt_from = datetime.strptime(window['Window_Start'].strip(), fmt)
        dt_to = datetime.strptime(window['Window_End'].strip(), fmt)
        current = dt_from
        while current < dt_to:
            d = current.date()
...
class SettlementEngine:
    ...
    def run(self):
        ...
        cap_gen = self.variables['Cap_Gen_KW']
        
        # Apply cap and zero out shutdown blocks for gen sum
        effective_gen_kws = []
        ...
        for d in self.date_list:
            for slot in range(1, 97):
                ...
                # Cons 1 block calc
                self._calc_block(d, slot, is_peak_block, is_shutdown, flat_kw_1, bank_inj_flat, bank_inj_iso, 
                                 self.cons1_data, self.iex1_data, self.variables['Con1_Label'], res1)
                
                # Cons 2 block calc
                self._calc_block(d, slot, is_peak_block, is_shutdown, flat_kw_2, bank_inj_flat, bank_inj_iso, 
                                 self.cons2_data, self.iex2_data, self.variables['Con2_Label'], res2)
        ...
        # Prepare Settlement Results dicts
        self._prepare_result(self.variables['Con1_Label'], s1, total_gen_kwh, active_blocks, flat_kw_1, avg_gen_kw, total_accountable, spilled_kw_1)
        self._prepare_result(self.variables['Con2_Label'], s2, total_gen_kwh, active_blocks, flat_kw_2, avg_gen_kw, total_accountable, spilled_kw_2)
    ...
    def _calc_block(self, d, slot, is_peak, is_shutdown, flat_kw, bank_inj_flat, bank_inj_iso, cons_data, iex_data, label, res_dict):
        c_raw = cons_data.get((d, slot), {'Apparent_KVA': 0, 'Active_KW_Raw': 0})
        kva = c_raw['Apparent_KVA']
        act_i_raw = c_raw['Active_KW_Raw']
        
        ct_ratio = self.variables['CT_Ratio']
        ...
        block_loss_pct = get_block_loss(d, slot, self.custom_losses, self.variables['Default_Loss'])
        ...
        gen_capped = min(raw_gen_kw, self.variables['Cap_Gen_KW']) if not is_shutdown else 0.0
    ...
    def _aggregate(self, res, flat_kw, active_blocks, bank_per_cons):
        ...
        prior_exit = sum((r['gen_share_kw'] + (bank_per_cons/active_blocks if r['gen_share_kw'] > 0 else 0)) * (1 - r['loss_pct']/100.0) for r in res.values()) / 4.0
        banked_entry = sum(r['bank_in_iso'] * (1 + r['loss_pct']/100.0) for r in res.values()) / 4.0
    ...
    def _prepare_result(self, label, s, total_gen_kwh, active_blocks, flat_kw, avg_gen_kw, total_accountable, spilled_kw):
        ...
        self.results.append({
            ...
            'Revised_Sch_At_Exit_KW': flat_kw * (1 - self.variables['Default_Loss']/100.0),
            'Max_Actual_KW': s['Max_Actual_KW'],
            'Cons_From_Gen_KW': flat_kw * (1 - self.variables['Default_Loss']/100.0),
            'Accountable_To_Discom_KW': s['Max_Actual_KW'] - (flat_kw * (1 - self.variables['Default_Loss']/100.0)),
