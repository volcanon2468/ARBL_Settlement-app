...
def build_shutdown_blocks(shutdown_windows: List[Dict]) -> Set[Tuple[date, int]]:
    blocks = set()
    fmt = "%d-%m-%Y %H:%M"
...
def get_block_loss(d: date, slot: int, custom_losses: List[Dict], default_loss: float) -> float:
...
class SettlementEngine:
    def __init__(self,
        ...
        self.gen_data = {(b['Block_Date'], b['Slot']): b['Active_KW'] for b in raw_gen_blocks}
        ...
        self.iex1_data = {(b['Block_Date'], b['Slot']): b['IEX_KW'] for b in raw_iex1_blocks}
        self.iex2_data = {(b['Block_Date'], b['Slot']): b['IEX_KW'] for b in raw_iex2_blocks}
        self.date_list = [start_date + timedelta(days=x) for x in range((end_date - start_date).days + 1)]
    ...
    def run(self):
        ...
        cap_gen = (self.variables.get('Cap_Gen_KW') or float('inf'))
        effective_gen_kws = []
        ...
        override_cap = (self.variables.get('Override_Capacity_MW') or 12.5)
        cap_kw = override_cap * 1000.0
        total_sched_kwh = cap_kw * (active_blocks / 4.0)
        total_gen_kwh = min(total_raw_gen_kwh, total_sched_kwh)
        avg_gen_kw = total_gen_kwh / (active_blocks / 4.0) if active_blocks > 0 else 0.0

        # Step 1: Compute each consumer's entitled schedule from their share %
        share1_kwh = total_gen_kwh * (self.variables['Share_Cons1'] / 100.0)
        share2_kwh = total_gen_kwh * (self.variables['Share_Cons2'] / 100.0)

        # Step 2: Convert entitled schedule to flat KW (before any physical cap)
        ...
        # Step 3: Apply physical CPP capacity cap per consumer.
        # Cap_Share_Cons1/2 is the fraction of total CPP MW that each consumer physically
        # holds (e.g. 25MW out of 50MW = 50%). This caps the scheduled flat KW to that
        # physical limit. Any entitled KWH above the cap is banked, not scheduled.
        override_cap = (self.variables.get('Override_Capacity_MW') or 12.5)
        cap_kw = override_cap * 1000.0
        cap_cons1 = cap_kw * ((self.variables.get('Cap_Share_Cons1') or 50.0) / 100.0)
        cap_cons2 = cap_kw * ((self.variables.get('Cap_Share_Cons2') or 50.0) / 100.0)

        spilled_kw_1 = 0.0
        spilled_kw_2 = 0.0

        # Consumer 1: if entitled flat KW exceeds physical cap, spill the excess
        if cap_cons1 and flat_kw_1 > cap_cons1:
            spilled_kw_1 = flat_kw_1 - cap_cons1
            flat_kw_1 = cap_cons1

        # Consumer 2: if entitled flat KW exceeds physical cap, spill the excess.
        # This is the key fix for Feb 2026 TPT2005:
        #   entitled flat KW = 8,125 KW (65% of avg 12,500 KW)
        #   physical cap      = 6,250 KW (25MW out of 50MW CPP)
        #   spilled_kw_2      = 1,875 KW → goes to banking
        #   flat_kw_2 (used for Prior Schedule) = 6,250 KW
        if cap_cons2 and flat_kw_2 > cap_cons2:
            spilled_kw_2 = flat_kw_2 - cap_cons2
            flat_kw_2 = cap_cons2

        # Step 4: Compute banking KWH for each consumer.
        # The spilled (excess entitled) KWH for each consumer is banked at entry point,
        # then reduced by the consumer's own transmission loss to arrive at exit-point banking.
        # Excel Banking Units cell = spilled_kwh_entry × (1 - loss_pct)
        loss_cons1 = (self.variables.get('Default_Loss_Cons1') or self.variables.get('Default_Loss') or 0.0) / 100.0
        loss_cons2 = (self.variables.get('Default_Loss_Cons2') or self.variables.get('Default_Loss') or 0.0) / 100.0

        banking_kwh_1_entry = (spilled_kw_1 * active_blocks) / 4.0
        banking_kwh_2_entry = (spilled_kw_2 * active_blocks) / 4.0
        banking_kwh_1_exit = banking_kwh_1_entry * (1 - loss_cons1)
        banking_kwh_2_exit = banking_kwh_2_entry * (1 - loss_cons2)

        # Old bank injection (previously banked units being drawn back this month)
        net_old_bank = self.variables['Old_Bank_KWH'] * (1 - (self.variables['Bank_Loss_Pct'] / 100.0))
        ...
        bank_inj_iso = (bank_per_cons / total_bank_blocks) * 4.0 if total_bank_blocks > 0 else 0.0
        ...
        for d in self.date_list:
            for slot in range(1, 97):
                ...
                self._calc_block(d, slot, is_peak_block, is_shutdown,
                                 flat_kw_1, bank_inj_flat, bank_inj_iso,
                                 self.cons1_data, self.iex1_data,
                                 self.variables['Con1_Label'], res1)
                self._calc_block(d, slot, is_peak_block, is_shutdown,
                                 flat_kw_2, bank_inj_flat, bank_inj_iso,
                                 self.cons2_data, self.iex2_data,
                                 self.variables['Con2_Label'], res2)
        ...
        total_accountable = s1['Energy_Accountable_To_Gen'] + s2['Energy_Accountable_To_Gen']

        self._prepare_result(self.variables['Con1_Label'], s1, total_gen_kwh, active_blocks,
                             flat_kw_1, avg_gen_kw, total_accountable, spilled_kw_1,
                             banking_kwh_1_exit)
        self._prepare_result(self.variables['Con2_Label'], s2, total_gen_kwh, active_blocks,
                             flat_kw_2, avg_gen_kw, total_accountable, spilled_kw_2,
                             banking_kwh_2_exit)
    ...
    def _calc_block(self, d, slot, is_peak, is_shutdown, flat_kw,
                    bank_inj_flat, bank_inj_iso, cons_data, iex_data, label, res_dict):
        c_raw = cons_data.get((d, slot), {'Apparent_KVA': 0, 'Active_KW_Raw': 0})
        kva = c_raw['Apparent_KVA']
        act_i_raw = c_raw['Active_KW_Raw']
        pf = round(act_i_raw / kva, 3) if kva > 0 else 1.0
        iex_power = iex_data.get((d, slot), 0.0)
        ct_ratio = (self.variables.get('CT_Ratio') or 1.0)
        ...
        block_loss_pct = get_block_loss(d, slot, self.custom_losses, (self.variables.get('Default_Loss') or 0.0))
        ...
        discom_kva_block = max(0.0, (actual_kw - net_gen_iso) / pf) if pf > 0 else 0.0

        raw_gen_kw = self.gen_data.get((d, slot), 0.0)
        gen_capped = min(raw_gen_kw, (self.variables.get('Cap_Gen_KW') or float('inf'))) if not is_shutdown else 0.0
    ...
    def _aggregate(self, res, flat_kw, active_blocks, bank_per_cons):
        ...
        # EBC injects bank power into all blocks (even peak and shutdown blocks)
        # for this intermediate exit schedule calculation
        bank_inj_flat = (bank_per_cons / active_blocks) * 4.0 if active_blocks > 0 else 0.0
        prior_exit = sum(
            (r['gen_share_kw'] + bank_inj_flat) * (1 - r['loss_pct'] / 100.0)
            for r in res.values()
        ) / 4.0

        banked_entry = sum(
            r['bank_in_iso'] * (1 + r['loss_pct'] / 100.0)
            for r in res.values()
        ) / 4.0
    ...
    def _prepare_result(self, label, s, total_gen_kwh, active_blocks,
                        flat_kw, avg_gen_kw, total_accountable, spilled_kw,
                        banking_kwh_exit):
        # The EBC math strictly defines Bank as (Allocated at Entry - Consumed at Entry)
        # which represents the unused Bank evaluated BEFORE line losses are applied
        ...
        self.results.append({
            ...
            'Banking_Units_Exit': banking_kwh_exit,
            ...
            'Revised_Sch_At_Exit_KW': flat_kw * (1 - loss_pct),
            'Cons_From_Gen_KW': flat_kw * (1 - loss_pct),
            'Accountable_To_Discom_KW': s['Max_Actual_KW'] - flat_kw * (1 - loss_pct),
